...
function login() {
  ...
  auth.signInWithEmailAndPassword(id + "@henrybankki.fi", pin)
    ...
    .catch(() => alert("Kirjautuminen epäonnistui."));
}
...
function signup() {
  ...
  auth.createUserWithEmailAndPassword(id + "@henrybankki.fi", pin)
    ...
    .catch(() => alert("Tiliä ei voitu luoda."));
}
...
function sendMoney() {
  ...
  db.runTransaction(async (transaction) => {
    ...
    if (!senderDoc.exists || !receiverDoc.exists) throw "Virhe";
  ...
  .catch(() => alert("Virhe rahansiirrossa."));
}
